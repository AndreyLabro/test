<?php
	...
	# 3 поста по 1/3 Старая книга и 1 широкий пост
	...
	while ( have_posts() ) : the_post();
		$i++;
		if ( $i > ( int )$posts_per_page ):
			$nomore = false;
			break;
		endif;
		$_post = $post;

		if( ! in_category( CAT_OLDBOOK ) ) : 
			$order[ 0 ][] = $post;
		else :
			$order[ 1 ][] = $post;
		endif;
	 ...
	 <div class="col-xs-<?php echo $width[0] ?> col-sm-<?php echo $width[1] ?> col-md-<?php echo $width[2] ?> grid-item">
	  <section class="mediaBlock -withBg -lighten -newBook">
		<div class="mediaBlockInfo">
			...
			<div class="mediaBlockInfo-category"><?php echo get_term_field( 'name', CAT_OLDBOOK ); ?></div>
		  <div style="background-image: url(<?php echo get_the_post_thumbnail_url( $_post->ID, 'large' ); ?>)" class="mediaBlockInfo-img"></div>
		  <div class="mediaBlockInfo-date"><?php the_field( 'old_book__date', $_post->ID ); ?></div>
          <?php if( get_field('partners', $_post->ID) ) : ?>
		...
		<?php endif; ?>
		  <div class="mediaBlockInfo-main">
            ...
            <a href="<?php echo get_permalink( $_post->ID ); ?>" class="mediaBlockInfo-title"><?php echo  get_the_title( $_post->ID ); ?></a> <div class="mediaBlockInfo-about md-size"><?php the_field('subtitle', $_post->ID ); ?></div>
		  </div>
		</div>
	  </section>
	</div>
	...
	function __write_wide_post_block( $_post, $width = array( '12', '12', '12' ) ){
		if ( ! in_category( CAT_CONTEXT, $_post->ID ) ) : // Контекст
	?>
	<div class="col-xs-<?php echo $width[0] ?> col-sm-<?php echo $width[1] ?> col-md-<?php echo $width[2] ?> grid-item" style="position: absolute; left: 0px; top: 0px;">
	  <section class="mediaBlock -withBg -fullWidth -sectionLong -default -lighten">
		<div class="mediaBlockInfo">
            <a href="<?php echo get_permalink( $_post->ID ); ?>" class="mediaBlockInfo-link-block"></a>
			<div style="background-image: url(<?php the_field('alt_img', $_post->ID ); ?>);" class="mediaBlockInfo-img"></div>
			<div class="mediaBlockInfo-date"><?php echo get_the_time( get_option( 'date_format' ), $_post->ID  ); ?></div>
			<div class="mediaBlockInfo-main">
				<div class="row clear">
				  <div class="col-md-9"><a href="<?php echo get_permalink( $_post->ID ); ?>" class="mediaBlockInfo-title"> <span>
						<q><?php echo  get_the_title( $_post->ID ); ?></q></span></a>
	...
	?>
			<div class="col-xs-<?php echo $width[0] ?> col-sm-<?php echo $width[1] ?> col-md-<?php echo $width[2] ?> grid-item <?php echo $block_count; ?>" style="position: absolute; left: 0px; top: 0px;">
			  <section class="mediaBlock -img-right -context">
				<div class="row">
				  <div class="hidden-xs col-md-4 col-sm-6">
					<div style="background-image: url(<?php echo get_the_post_thumbnail_url( $_post->ID, 'large' ); ?>); <?php if( get_field('edit_alignment', $_post->ID) ) : echo 'background-position: ' ?><?php echo get_field('vertical_alignment', $_post->ID) . ' ' . get_field('horizontal_alignment', $_post->ID) . ' !important'; ?><?php endif; ?>" class="mediaBlockInfo-img"></div>
				  </div>
				  <div class="col-md-8 col-sm-6">
					<div class="mediaBlockInfo mediaBlockInfo-default <?php echo _get_author_full_name( $_post->ID ); ?>">
                        <a href="<?php echo get_permalink( $_post->ID ); ?>" class="mediaBlockInfo-link-block"></a>
						<?php if( _get_author_full_name( $_post->ID ) != ' ') : ?>						
						<a href="<?php echo _get_author_link( $_post->ID ); ?>" class="mediaBlockInfo-author">
						<?php echo _get_author_full_name( $_post->ID ); ?>
						</a>
						<?php endif; ?>
						
						<div style="background-image: url(<?php the_field('alt_img', $_post->ID ); ?>);" class="visible-xs mediaBlockInfo-img"></div>
						<div class="mediaBlockInfo-date"><?php echo get_the_date( '',$_post->ID); ?></div>
                        <?php if( get_field('partners', $_post->ID) ) : ?>
		<div class="partner-block">
			<label class="partner-label">Партнерский материал</label>
		...
		<?php endif; ?>
						<div class="mediaBlockInfo-main">
                            ...
                            <a href="<?php the_permalink( $_post->ID ); ?>" class="mediaBlockInfo-title"><?php echo get_the_title( $_post->ID ); ?></a>
							<div class="mediaBlockInfo-about"><?php the_field( 'subtitle', $_post->ID); ?></div>
						</div>
					</div>
				  </div>
